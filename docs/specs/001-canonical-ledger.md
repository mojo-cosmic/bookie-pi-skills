...
- `policy.evidence_roots` is a required, non-empty, duplicate-free array of lexical relative POSIX paths. Each path consists of non-empty slash-separated segments. Absolute paths, empty segments, `.` or `..` segments, backslashes, glob metacharacters, colons (including Windows drive and URI-like forms), percent-encoded forms, all C0/C1 control characters, and lone UTF-16 surrogates are rejected. Valid Unicode scalar path characters remain intentionally supported. This is only a lexical schema boundary: filesystem containment, normalization against a vault root, and realpath/symlink handling remain runtime policy and are not claimed here.
...
**`SENSITIVITY-ALLOWLIST`** (implementation deferred to SPEC-002/BK-007): indexing, checkpointing, logging, and exporting MUST proceed only when a record has a sensitivity class that is declared in `policy.sensitivity.classes`, is not reserved, and is absent from `policy.sensitivity.excluded_classes`. A missing, reserved, undeclared, or excluded class MUST deny each operation. Runtime profile validation MUST also reject an `excluded_classes` entry that is not declared in `classes`; affected operations remain denied while the profile is invalid. This fail-closed cross-array and record-policy rule is deliberately not attributed to JSON Schema enforcement.
...
The schema is a contract, not the complete policy engine. `SENSITIVITY-ALLOWLIST` above is a named deferred runtime rule; target existence, inverse relations, Git-base immutability, and resource digests are documented as additional named validation rules for SPEC-002.
